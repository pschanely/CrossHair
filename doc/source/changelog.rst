 ...
 * Fix several ``array.array`` divergences in the symbolic implementation.
   ...
   on unsigned arrays, just like CPython. Value bounds for the ``i``, ``I``,
   ``l``, and ``L`` typecodes now match the platform's item size (they were
